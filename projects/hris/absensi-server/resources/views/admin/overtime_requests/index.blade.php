...
<div class="admin-card">
    <div class="p-6 border-b border-slate-50 flex flex-col md:flex-row justify-between items-center gap-4">
        <h4 class="font-bold text-slate-800">Daftar Pengajuan Lembur</h4>
        
        <form action="{{ route('admin.overtime_requests.index') }}" method="GET" class="flex flex-wrap items-center gap-1.5">
            <!-- Search -->
            <div class="relative">
            ...
            @if(request()->anyFilled(['search', 'status', 'month', 'year']))
                <a href="{{ route('admin.overtime_requests.index') }}" class="bg-slate-100 text-slate-400 hover:text-red-500 transition-colors p-2 rounded-xl" title="Reset Filter">
        ...
        <table class="w-full text-left">
            <thead>
                <tr class="admin-table-thead">
                    ...
                    <th class="px-6 py-4">Tipe Lembur</th>
                    <th class="px-6 py-4 text-center">Waktu</th>
                    <th class="px-6 py-4 text-center">Istirahat</th>
            ...
            <tbody class="divide-y divide-slate-50">
                ...
                <tr class="admin-table-row">
                    ...
                    <td class="px-6 py-4 text-center text-sm font-medium text-slate-800">
                        {{ $it->start_at->format('d M Y, H:i') }} - {{ $it->end_at->format('H:i') }}
                    </td>
                    ...
                    <td class="px-6 py-4 text-right">
                        ...
                        <div class="flex justify-end gap-2">
                            <form action="{{ route('admin.overtime_requests.status', $it) }}" method="POST" onsubmit="confirmAction(event, 'Setujui pengajuan lembur ini?')">
                                @csrf
                                @method('PATCH')
                            ...
                            <form action="{{ route('admin.overtime_requests.status', $it) }}" method="POST" onsubmit="confirmAction(event, 'Tolak pengajuan lembur ini?')">
                                @csrf
                                @method('PATCH')
