...
<div class="admin-card">
    <div class="p-6 border-b border-slate-50 flex flex-col md:flex-row justify-between items-center gap-4">
        <h4 class="font-bold text-slate-800">Daftar Pengajuan Cuti</h4>
        
        <form action="{{ route('admin.leave_requests.index') }}" method="GET" class="flex flex-wrap items-center gap-1.5">
            <!-- Search -->
            <div class="relative">
            ...
            <select name="type" class="admin-input w-32">
                ...
                <option value="Cuti Tahunan" {{ request('type') == 'Cuti Tahunan' ? 'selected' : '' }}>Cuti Tahunan</option>
                <option value="Cuti Melahirkan" {{ request('type') == 'Cuti Melahirkan' ? 'selected' : '' }}>Cuti Melahirkan</option>
            ...
            @if(request()->anyFilled(['search', 'status', 'type', 'month', 'year']))
                <a href="{{ route('admin.leave_requests.index') }}" class="bg-slate-100 text-slate-400 hover:text-red-500 transition-colors p-2 rounded-xl" title="Reset Filter">
            ...
            <tbody class="divide-y divide-slate-50">
                ...
                <tr class="admin-table-row">
                    ...
                    <td class="px-6 py-4 text-center">
                        <div class="text-sm font-medium text-slate-800 leading-none">{{ $it->start_date->format('d M Y') }}</div>
                        <div class="text-[10px] text-slate-400 mt-1">s/d {{ $it->end_date->format('d M Y') }}</div>
                    </td>
                    <td class="px-6 py-4 text-center">
                    ...
                    <td class="px-6 py-4 text-right">
                        ...
                        <div class="flex justify-end gap-2">
                            <form action="{{ route('admin.leave_requests.status', $it) }}" method="POST" onsubmit="confirmAction(event, 'Setujui pengajuan cuti ini?')">
                                @csrf
                                @method('PATCH')
                            ...
                            <form action="{{ route('admin.leave_requests.status', $it) }}" method="POST" onsubmit="confirmAction(event, 'Tolak pengajuan cuti ini?')">
                                @csrf
                                @method('PATCH')
