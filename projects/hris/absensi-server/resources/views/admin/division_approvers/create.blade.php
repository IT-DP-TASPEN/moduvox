...
<div class="flex items-center gap-4">
    ...
    <div>
        <h1 class="text-2xl font-bold text-slate-800">Tambah Setting Approval</h1>
    </div>
</div>
@endsection

@section('content')
<form action="{{ route('admin.division-approvers.store') }}" method="POST" class="space-y-6">
    @csrf
                ...
                <div class="relative">
                    ...
                    <select name="division_name" id="division_select" required class="w-full px-4 py-3 rounded-2xl border border-slate-200 focus:ring-2 focus:ring-brand-blue outline-none transition-all">
                        ...
                        @foreach($divisions as $division)
                            <option value="{{ $division->name }}" {{ old('division_name') == $division->name ? 'selected' : '' }}>{{ $division->name }}</option>
                        @endforeach
                    </select>
                </div>
            ...
            <div class="space-y-2">
                ...
                <div class="relative">
                    ...
                    <select name="approver_id" id="approver_select" class="w-full px-4 py-3 rounded-2xl border border-slate-200 focus:ring-2 focus:ring-brand-blue outline-none transition-all">
                        ...
                        @foreach($users as $u)
                            <option value="{{ $u->id }}" {{ old('approver_id') == $u->id ? 'selected' : '' }}>{{ $u->name }} ({{ $u->title }})</option>
                        @endforeach
                    </select>
                </div>
            ...
            <div class="space-y-2">
                ...
                <div class="relative">
                    ...
                    <select name="director_id" id="director_select" class="w-full px-4 py-3 rounded-2xl border border-slate-200 focus:ring-2 focus:ring-brand-blue outline-none transition-all">
                        ...
                        @foreach($users as $u)
                            <option value="{{ $u->id }}" {{ old('director_id') == $u->id ? 'selected' : '' }}>{{ $u->name }} ({{ $u->title }})</option>
                        @endforeach
                    </select>
                </div>
