...
@section('header', 'Pengajuan Tugas Luar')

@section('content')
<div class="admin-card">
    <div class="p-6 border-b border-slate-50 flex flex-col md:flex-row justify-between items-center gap-4">
        <h4 class="font-bold text-slate-800">Daftar Pengajuan Tugas Luar</h4>
        
        <form action="{{ route('admin.outside_duty_requests.index') }}" method="GET" class="flex flex-wrap items-center gap-1.5">
            <!-- Search -->
            <div class="relative">
            ...
            @if(request()->anyFilled(['search', 'status', 'month', 'year']))
                <a href="{{ route('admin.outside_duty_requests.index') }}" class="bg-slate-100 text-slate-400 hover:text-red-500 transition-colors p-2 rounded-xl" title="Reset Filter">
        ...
        <table class="w-full text-left">
            <thead>
                <tr class="admin-table-thead">
                    ...
                    <th class="px-6 py-4 text-center">Lembur/Break</th>
            ...
            <tbody class="divide-y divide-slate-50">
                ...
                <tr class="admin-table-row">
                    ...
                    <td class="px-6 py-4 text-center text-sm font-medium text-slate-800">
                        <div class="leading-none">{{ $it->start_at->format('d M Y') }}</div>
                        <div class="text-[10px] text-slate-400 mt-1">{{ $it->start_at->format('H:i') }} - {{ $it->end_at->format('H:i') }}</div>
                    </td>
                    <td class="px-6 py-4 text-center text-[10px] font-bold text-slate-500">
                        L: {{ $it->overtime_minutes }}m / B: {{ $it->break_minutes }}m
                    </td>
                    ...
                    <td class="px-6 py-4 text-center">
                        ...
                        <a href="https://www.google.com/maps/search/?api=1&query={{ $it->latitude }},{{ $it->longitude }}" target="_blank" class="inline-flex items-center gap-1 text-[10px] font-black text-rose-600 hover:text-rose-800 uppercase bg-rose-50 px-2 py-1 rounded-lg transition-colors">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <td class="px-6 py-4 text-right">
                        ...
                        <div class="flex justify-end gap-2">
                            <form action="{{ route('admin.outside_duty_requests.status', $it) }}" method="POST" onsubmit="confirmAction(event, 'Setujui pengajuan tugas luar ini?')">
                                @csrf
                                @method('PATCH')
                            ...
                            <form action="{{ route('admin.outside_duty_requests.status', $it) }}" method="POST" onsubmit="confirmAction(event, 'Tolak pengajuan tugas luar ini?')">
                                @csrf
                                @method('PATCH')
