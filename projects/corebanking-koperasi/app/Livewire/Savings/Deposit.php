...
class Deposit extends Component
{
    ...
    public $description = '';
    public $channel = 'CASH';
    public $bank_coa_id   = null;  // Sub-akun ABA spesifik
    public $cash_coa_id   = null;  // Sub-akun Kas spesifik
    public $coa_id = null;
    public $coaSearch = '';
    ...
    public function selectAccount($id)
    {
        ...
        $this->description = 'Setoran Simpanan';

        $this->logActivity('DEPOSIT_SELECT_ACCOUNT', "Memilih rekening ID [{$id}] untuk setoran");
    }
    ...
    public function submit()
    {
        ...
        if ($account->status !== 'ACTIVE' && $account->status !== 'PENDING') {
            $this->addError('amount', "Rekening sedang dalam status {$account->status}. Transaksi tidak diizinkan.");
            return;
        }

        if ($account->status === 'PENDING' && $this->amount < $account->product->min_initial_deposit) {
            $minFormatted = number_format($account->product->min_initial_deposit, 0, ',', '.');
            $this->addError('amount', "Setoran awal minimum untuk produk ini adalah Rp {$minFormatted}");
            return;
        }
        ...
        $status = $this->interceptAction('savings.deposit', 'DEPOSIT', $data);

        $this->logActivity('DEPOSIT_REQUEST', "Mengajukan setoran Rp " . number_format($this->amount, 2, ',', '.') . " untuk rekening [{$account->account_no}]");

        if ($status === 'PENDING') {
            session()->flash('success', 'Transaksi setoran telah diajukan ke antrean persetujuan.');
        } else {
            $service = app(\App\Services\SavingOperationService::class);
            $service->postTransaction($account, 'DEPOSIT', $this->amount, $this->description, $this->reference_no, $this->channel, $coaOverrideId);
            session()->flash('success', 'Setoran berhasil diposting.');
        }
    ...
    public function mount()
    {
        ...
        $this->updatedChannel($this->channel);
        $this->logActivity('NAVIGATE', 'Setoran Simpanan');
    }
    ...
    public function render()
    {
        ...
        return view('livewire.savings.deposit', [
            'items' => $items,
