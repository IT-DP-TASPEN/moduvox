<div class="p-0">
    <x-header title="Blokir Rekening CIF" subtitle="Bekukan seluruh interaksi dan hak akses operasional pihak terkait." :user="auth()->user()" :role="auth()->user()->getRoleNames()->first() ?? 'No Role'">
        <x-slot:actions>
    ...
    <div class="p-10">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        ...
                        <tr class="hover:bg-slate-50/50 transition-colors">
                            <td class="py-4 px-6 relative w-24">
                                <button wire:click="viewCif({{ $item->id }})" class="flex items-center text-[10px] uppercase tracking-wider font-bold px-3 py-1.5 bg-rose-50 text-rose-600 hover:bg-rose-600 hover:text-white rounded-lg transition-all"><span class="material-symbols-outlined text-[14px] mr-1">lock</span> Blokir</button>
                            </td>
                        ...
                        <tr>
                            <td colspan="5" class="py-24 text-center text-slate-400">
                                ...
                                <p class="text-sm font-bold">Lakukan pencarian untuk memunculkan target Blokir.</p>
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
        ...
        @else
        <!-- BLOCK FORM -->
        <div class="bg-white rounded-[2rem] shadow-sm border border-rose-200/60 overflow-hidden flex flex-col mb-10 ring-4 ring-rose-50">
            <div class="px-8 py-6 bg-rose-50 border-b border-rose-100 flex items-center space-x-4">
                <button wire:click="closeView" class="p-2 bg-white text-rose-600 hover:bg-rose-200 rounded-xl transition-all shadow-sm border border-rose-200"><span class="material-symbols-outlined text-sm">arrow_back</span></button>
                <div>
                    <h2 class="font-extrabold text-sm text-rose-900 tracking-wider flex items-center"><span class="material-symbols-outlined mr-2">security_update_warning</span> Eksekusi Pemblokiran CIF: {{ $selectedCif->cif_no ?? '' }}</h2>
                </div>
            </div>
            ...
            <div class="p-8 pb-12 space-y-8">
                <div>
                    <div class="border-b border-slate-200 pb-2 mb-6">
                        <p class="text-xs font-bold text-rose-600 uppercase tracking-widest">Detail Dasar Pemblokiran</p>
                    </div>
                    <div class="space-y-2">
                        <label class="text-[10px] uppercase tracking-widest font-extrabold text-slate-400 ml-1">Keterangan / Alasan Blokir <span class="text-rose-500">*</span></label>
                        <textarea wire:model="reason" rows="3" placeholder="Masukkan alasan mengapa pengguna ini masuk daftar tunggu blokir..." class="w-full px-5 py-3.5 bg-white border border-rose-200 rounded-2xl focus:ring-4 focus:ring-rose-900/5 focus:border-rose-500 transition-all font-bold text-sm text-slate-700 resize-none"></textarea>
                        @error('reason') <span class="text-[10px] text-rose-500 font-bold ml-1">{{ $message }}</span> @enderror
                    </div>
                </div>
            </div>
            
            <div class="px-8 py-5 border-t border-rose-100 bg-rose-50 flex items-center justify-between">
                <p class="text-xs font-bold text-rose-600 uppercase tracking-widest flex items-center"><span class="material-symbols-outlined text-sm mr-2 animate-pulse">warning</span> Aksi ini akan melarang nasabah melakukan transaksi jenis apapun.</p>
                <button wire:click="submitBlock" class="px-8 py-3 bg-rose-600 text-white font-bold text-xs rounded-xl hover:bg-rose-700 hover:shadow-lg hover:shadow-rose-500/30 transition-all flex items-center">
                    <span class="material-symbols-outlined text-sm mr-2">lock</span> Konfirmasi Blokir
                </button>
            </div>
        </div>
        @endif
