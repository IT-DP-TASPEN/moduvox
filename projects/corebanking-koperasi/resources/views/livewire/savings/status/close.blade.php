<div class="p-0">
    <x-header title="Penutupan Rekening" subtitle="Pencarian rekening simpanan untuk pengakhiran keanggotaan" :user="$user" :role="$role">
        <x-slot:actions>
    ...
    <div class="p-10">
                        ...
                        <tr class="hover:bg-slate-50/50 transition-colors group">
                             <td class="py-4 px-6">
                                <div class="flex items-center justify-center">
                                    <button wire:click="selectAccount({{ $item->id }})" class="p-2 bg-white text-slate-600 hover:bg-slate-50 rounded-xl shadow-sm border border-slate-200 transition-all hover:text-slate-900" title="Tutup Rekening">
                            ...
                            <td class="py-4 px-6 text-center">
                                <span class="px-3 py-1 text-[10px] font-black uppercase tracking-wider rounded-lg {{ $item->status == 'ACTIVE' ? 'bg-emerald-50 text-emerald-500 border border-emerald-100' : 'bg-amber-50 text-amber-500 border border-amber-100' }}">
                                    {{ $item->status }}
                                </span>
                            </td>
                        </tr>
        ...
        <div class="space-y-8 animate-in slide-in-from-bottom-6 duration-700 text-left">
            <!-- Account Info Card -->
            <div class="bg-white rounded-[2rem] shadow-sm border border-slate-200/60 overflow-hidden">
                <div class="p-8 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center text-rose-600 font-black uppercase tracking-widest text-xs">
                    <div class="flex items-center space-x-4">
                        ...
                        <div>
                            <h3 class="text-sm font-black text-slate-900 tracking-tight">{{ $selectedAccount->cif->name }}</h3>
                            <p class="text-[10px] text-slate-500 font-bold tracking-widest leading-none mt-1 uppercase">{{ $selectedAccount->account_no }} • {{ $selectedAccount->product->name }}</p>
                        </div>
                    </div>
                    <div class="px-4 py-2 bg-rose-50 border border-rose-100 rounded-xl text-center shadow-sm">
                        <p class="text-[8px] font-black text-rose-400 leading-none mb-1">Status Rekening</p>
                        <p class="text-[10px] font-black text-rose-600">{{ $selectedAccount->status }}</p>
                    </div>
                </div>
                ...
                <div class="p-10 grid grid-cols-1 md:grid-cols-2 gap-10">
                    <div class="space-y-6">
                        <div class="border-b border-slate-100 pb-2">
                            <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none">Keanggotaan</p>
                        </div>
                        <div class="grid grid-cols-1 gap-4">
                            <div class="space-y-1">
                                <p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest leading-none">NIK Peserta</p>
                                <p class="text-xs font-bold text-slate-900">{{ $selectedAccount->cif->nik }}</p>
                            </div>
                            <div class="space-y-1">
                                <p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest leading-none">Domisili</p>
                                <p class="text-xs font-bold text-slate-900 leading-relaxed italic">"{{ $selectedAccount->cif->address }}"</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="space-y-6">
                        <div class="border-b border-slate-100 pb-2">
                            <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none">Kewajiban Penutupan</p>
                        </div>
                        <div class="p-8 bg-rose-50/50 rounded-[2rem] border border-rose-100 flex flex-col items-center justify-center text-center space-y-2">
                            <p class="text-[10px] font-black text-rose-400 uppercase tracking-[0.2em]">Saldo Tersisa</p>
                            <p class="text-3xl font-black text-rose-600 tracking-tighter">Rp {{ number_format($selectedAccount->balance, 2, ',', '.') }}</p>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="space-y-8">
                    <div class="border-b border-slate-100 pb-2">
                        <p class="text-xs font-bold text-slate-900 uppercase tracking-widest flex items-center gap-2">
                            <span class="material-symbols-outlined text-sm text-slate-400">cancel</span> 
                            Form Pengajuan Penutupan Simpanan
                        </p>
                    </div>

                    <div class="space-y-6">
                        <div class="space-y-4">
                            <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Alasan Penutupan</label>
                            <textarea wire:model="reason" rows="3" class="w-full px-6 py-4 bg-slate-50 border border-slate-200 rounded-2xl text-xs font-bold focus:outline-none focus:ring-2 focus:ring-slate-900/10 focus:border-slate-900 transition-all font-medium" placeholder="Sebutkan alasan mengapa anggota ingin menutup rekening simpanan ini..."></textarea>
                        ...
                        <div class="p-6 bg-rose-50 border border-rose-100 rounded-2xl flex items-start space-x-4">
                            <span class="material-symbols-outlined text-rose-500 mt-1">warning</span>
                            <div class="space-y-1">
                                <p class="text-[10px] font-black text-rose-600 uppercase tracking-widest leading-none">PERINGATAN PENUTUPAN</p>
                                <p class="text-[10px] font-medium text-rose-700 leading-relaxed italic opacity-70">
                                    Status penutupan bersifat PERMANEN. Sebelum diproses, saldo rekening HARUS bernilai Rp 0 (Nol). 
                                </p>
                            </div>
                        </div>
                        ...
                        <div class="pt-8 border-t border-slate-100 flex justify-between items-center">
                            <div class="flex items-center space-x-3 text-slate-400">
                                ...
                                <p class="text-[9px] font-bold uppercase tracking-widest leading-none">Memerlukan Persetujuan Kepala Cabang</p>
                            </div>
