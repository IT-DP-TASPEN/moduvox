<div class="p-0">
    <x-header title="Nonaktifkan (Dormant)" subtitle="Pencarian rekening aktif untuk diubah menjadi pasif" :user="$user" :role="$role">
        <x-slot:actions>
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="border-b border-slate-200 bg-slate-50/50">
                            ...
                            <th class="py-5 px-6 text-[10px] font-extrabold tracking-widest text-slate-400 uppercase text-right">Saldo Saat Ini</th>
                        ...
                        <tr class="hover:bg-slate-50/50 transition-colors group">
                             <td class="py-4 px-6">
                                <div class="flex items-center justify-center">
                                    <button wire:click="selectAccount({{ $item->id }})" class="p-2 bg-white text-slate-600 hover:bg-slate-50 rounded-xl shadow-sm border border-slate-200 transition-all hover:text-slate-900" title="Nonaktifkan Rekening">
                            ...
                            <td class="py-4 px-6 text-center">
                                <span class="px-3 py-1 text-[10px] font-black uppercase tracking-wider rounded-lg bg-emerald-100 text-emerald-700">
                                    {{ $item->status }}
                                </span>
                            </td>
                        </tr>
        ...
        <div class="max-w-4xl mx-auto space-y-8 animate-in slide-in-from-bottom-6 duration-700">
            ...
            <div class="bg-white rounded-[2rem] shadow-sm border border-slate-200/60 overflow-hidden">
                <div class="p-8 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center text-indigo-600">
                    ...
                    <div class="px-4 py-2 bg-emerald-50 border border-emerald-100 rounded-xl text-center shadow-sm">
                        <p class="text-[8px] font-black text-emerald-400 uppercase leading-none mb-1">Status Saat Ini</p>
                ...
                <div class="p-10 grid grid-cols-1 md:grid-cols-2 gap-10 text-left">
                    <div class="space-y-6">
                        <div class="border-b border-slate-100 pb-2">
                            <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Data Anggota</p>
                        </div>
                        <div class="grid grid-cols-1 gap-4">
                            ...
                            <div class="space-y-1">
                                <p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest leading-none">Alamat Sesuai Identitas</p>
                    ...
                    <div class="space-y-6">
                        <div class="border-b border-slate-100 pb-2">
                            <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Informasi Saldo Akhir</p>
                        </div>
                        <div class="p-8 bg-slate-100 rounded-[2rem] border border-slate-200 flex flex-col items-center justify-center text-center space-y-2">
                            <p class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Saldo Tersedia</p>
                            <p class="text-3xl font-black text-slate-900 tracking-tighter">Rp {{ number_format($selectedAccount->balance, 2, ',', '.') }}</p>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="space-y-8">
                    <div class="border-b border-slate-100 pb-2 text-left">
                        <p class="text-xs font-bold text-slate-900 uppercase tracking-widest flex items-center gap-2">
                            <span class="material-symbols-outlined text-sm text-slate-400">snooze</span> 
                            Form Pengalihan Status Dormant (Pasif)
                        </p>
                    </div>

                    <div class="space-y-6 text-left">
                        <div class="space-y-4">
                            <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Keterangan / Alasan Pasif</label>
                            <textarea wire:model="reason" rows="3" class="w-full px-6 py-4 bg-slate-50 border border-slate-200 rounded-2xl text-xs font-bold focus:outline-none focus:ring-2 focus:ring-slate-900/10 focus:border-slate-900 transition-all font-medium" placeholder="Jelaskan alasan pengalihan status menjadi pasif (dormant)..."></textarea>
                            @error('reason') <p class="text-[10px] text-rose-500 font-bold uppercase tracking-widest ml-1 italic">{{ $message }}</p> @enderror
                        </div>

                        <div class="p-6 bg-amber-50 border border-amber-100 rounded-2xl flex items-start space-x-4">
                            <span class="material-symbols-outlined text-amber-500 mt-0.5">info_i</span>
                            <div class="space-y-1">
                                <p class="text-[10px] font-black text-amber-600 uppercase tracking-widest leading-none">Status Pasif (Dormant)</p>
                                <p class="text-[10px] font-medium text-amber-700 leading-relaxed italic opacity-70">
                                    Rekening dengan status DORMANT tidak dapat melakukan transaksi penarikan dana sebelum dilakukan Reaktivasi oleh petugas.
                                </p>
                            </div>
                        </div>
