<div class="p-0">
    <x-header title="Setoran Simpanan" subtitle="Pencarian rekening dan input transaksi setoran" :user="$user"
        :role="$role">
        <x-slot:actions>
            <div class="flex items-center space-x-3">
                ...
                <div class="relative">
                    ...
                    <input wire:model.live.debounce.500ms="search" type="text"
                        placeholder="Cari No. Rekening, NIK, Nama..."
                ...
                <button wire:click="closeView"
                    class="flex items-center space-x-2 bg-white text-slate-900 border border-slate-200 px-4 py-2 rounded-xl font-bold text-xs hover:bg-slate-50 transition-all">
    ...
    <div class="p-10">
        ...
        <div
            class="bg-emerald-50 text-emerald-700 px-6 py-4 rounded-[2rem] border border-emerald-100 flex items-center mb-10 animate-fade-in shadow-sm">
                        ...
                        <tr wire:key="deposit-row-{{ $item->id }}" class="hover:bg-slate-50/50 transition-colors group">
                            <td class="py-4 px-6">
                                <div class="flex items-center justify-center">
                                    <a href="{{ route('savings.deposit', ['account' => $item->account_no]) }}" wire:navigate
                            ...
                            <td class="py-4 px-6">
                                <p class="font-bold text-sm text-slate-900 uppercase leading-none mb-1">{{
                                    $item->cif->name }}</p>
                                <p class="text-[10px] text-slate-500 font-bold tracking-widest">NIK: {{ $item->cif->nik
                                    }}</p>
                            </td>
                            ...
                            <td class="py-4 px-6">
                                <p class="font-bold text-[10px] text-slate-600 uppercase tracking-widest">{{
                                    $item->product->name }}</p>
                            </td>
                            <td class="py-4 px-6">
                                <span
                                    class="px-3 py-1 text-[10px] font-bold uppercase tracking-wider rounded-lg {{ $item->status == 'ACTIVE' ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700' }}">
                                    {{ $item->status }}
                                </span>
                            </td>
                        </tr>
        ...
        <div class="space-y-8 animate-in slide-in-from-bottom-6 duration-700">
            ...
            <div class="bg-white rounded-[2rem] shadow-sm border border-slate-200/60 overflow-hidden">
                <div class="p-8 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center">
                    <div class="flex items-center space-x-4">
                        <div
                            class="w-12 h-12 rounded-2xl bg-white border border-slate-200 flex items-center justify-center text-slate-400 shadow-sm">
                            <span class="material-symbols-outlined">person</span>
                        </div>
                        <div>
                            <h3 class="text-sm font-black text-slate-900 uppercase tracking-tight">{{
                                $selectedAccount->cif->name }}</h3>
                            <p class="text-[10px] text-slate-500 font-bold uppercase tracking-widest">{{
                                $selectedAccount->account_no }} • {{ $selectedAccount->product->name }}</p>
                        </div>
                    </div>
                    <div class="px-4 py-2 bg-emerald-50 border border-emerald-100 rounded-xl text-center">
                        <p class="text-[8px] font-black text-emerald-400 uppercase leading-none mb-1">Status Rekening
                        </p>
                ...
                <div class="p-10 grid grid-cols-1 md:grid-cols-2 gap-10">
                    <div class="space-y-6">
                        <div class="border-b border-slate-100 pb-2">
                            <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Informasi Anggota
                            </p>
                        </div>
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <div class="space-y-1">
                                <p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest leading-none">
                                    Telepon</p>
                                <p class="text-xs font-bold text-slate-900">{{ $selectedAccount->cif->phone ?? '-' }}
                                </p>
                            </div>
                    ...
                    <div class="space-y-6">
                        <div class="border-b border-slate-100 pb-2">
                            <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Saldo Saat Ini
                            </p>
                        </div>
                        <div class="p-6 bg-slate-50 rounded-2xl border border-slate-100 space-y-4">
                            <div class="flex justify-between items-center">
                                <p class="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Saldo Akhir
                                </p>
                                <p class="text-sm font-black text-slate-900">Rp {{
                                    number_format($selectedAccount->balance, 2, ',', '.') }}</p>
                            </div>
                            <div class="flex justify-between items-center">
                                <p class="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Saldo Blokir
                                </p>
                                <p class="text-sm font-black text-rose-500">Rp {{
                                    number_format($selectedAccount->blocked_balance ?? 0, 2, ',', '.') }}</p>
                            </div>
                            <div class="pt-3 border-t border-slate-200 flex justify-between items-center">
                                <p class="text-[10px] font-black text-slate-900 uppercase tracking-widest">Saldo Efektif
                                </p>
                                <p class="text-lg font-black text-slate-900">Rp {{
                                    number_format($selectedAccount->effective_balance, 2, ',', '.') }}</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="bg-white rounded-[2rem] shadow-sm border border-slate-200/60 p-10 mt-10">
                <div class="space-y-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
                        <div class="space-y-4">
                            ...
                            <div class="relative" x-data="{ 
                                ...
                                <input type="text" x-model="display"
                                    ...
                                    class="w-full pl-12 pr-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-black focus:outline-none focus:ring-2 focus:ring-slate-900/10 focus:border-slate-900 transition-all text-slate-900"
                                    placeholder="0">
                            </div>
                            @error('amount') <p
                                class="text-[10px] text-rose-500 font-bold uppercase tracking-widest ml-2 italic">{{
                                $message }}</p> @enderror
                        </div>
                        ...
                        <div class="space-y-4">
                            <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Jalur Transaksi</label>
                            <div class="relative">
                                <span class="material-symbols-outlined absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 text-sm">account_balance</span>
                                <select wire:model.live="channel" class="w-full pl-12 pr-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl text-xs font-black text-slate-900 focus:outline-none focus:ring-2 focus:ring-slate-900/10 focus:border-slate-900 transition-all appearance-none cursor-pointer">
                            ...
                            @error('channel') <p class="text-[10px] text-rose-500 font-bold uppercase tracking-widest ml-2 italic">{{ $message }}</p> @enderror
                        </div>
                        ...
                        <div class="space-y-4 animate-in fade-in slide-in-from-top-4 duration-300">
                            ...
                            <div class="relative">
                                ...
                                <select wire:model="bank_coa_id" class="w-full pl-12 pr-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl text-xs font-black text-slate-900 focus:outline-none focus:ring-2 focus:ring-slate-900/10 focus:border-slate-900 transition-all appearance-none cursor-pointer">
                            ...
                            @error('bank_coa_id') <p class="text-[10px] text-rose-500 font-bold uppercase tracking-widest ml-2 italic">{{ $message }}</p> @enderror
                        </div>
                        ...
                        <div class="space-y-4 animate-in fade-in slide-in-from-top-4 duration-300">
                            ...
                            <div class="relative">
                                ...
                                <select wire:model="cash_coa_id" class="w-full pl-12 pr-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl text-xs font-black text-slate-900 focus:outline-none focus:ring-2 focus:ring-slate-900/10 focus:border-slate-900 transition-all appearance-none cursor-pointer">
                            ...
                            @error('cash_coa_id') <p class="text-[10px] text-rose-500 font-bold uppercase tracking-widest ml-2 italic">{{ $message }}</p> @enderror
                        </div>
                        @elseif($channel === 'COA')
                        <div class="space-y-4 animate-in fade-in slide-in-from-top-4 duration-300">
                            <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">COA Transaksi <span class="text-rose-500">*</span></label>
                            <div class="relative">
                                ...
                                <datalist id="deposit-coa-options">
                            ...
                            @error('coa_id') <p class="text-[10px] text-rose-500 font-bold uppercase tracking-widest ml-2 italic">{{ $message }}</p> @enderror
                        </div>
                        @endif
                        <div class="space-y-4 md:col-span-2">
                            <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Keterangan Transaksi</label>
                            <input wire:model="description" type="text"
                                class="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl text-xs font-bold focus:outline-none focus:ring-2 focus:ring-slate-900/10 focus:border-slate-900 transition-all"
                                placeholder="Contoh: Setoran via Teller">
                    ...
                    <div class="pt-8 border-t border-slate-100 flex justify-between items-center">
                        <div class="flex items-center space-x-3 text-slate-400">
                            ...
                            <p class="text-[9px] font-bold uppercase tracking-widest">Otorisasi Supervisor Diperlukan
                            </p>
                        </div>
                        <button wire:click="submit"
                            class="bg-slate-900 hover:shadow-lg hover:shadow-slate-900/20 text-white px-12 py-4 rounded-xl font-bold text-xs uppercase tracking-widest transition-all active:scale-95 flex items-center space-x-3 group">
                            <span class="material-symbols-outlined text-sm">send</span>
                            <span>Posting Setoran</span>
                        </button>
                    </div>
                </div>
            </div>
            ...
            <div class="p-5 bg-slate-50 border border-slate-200 rounded-2xl flex items-center space-x-4">
                ...
                <p class="text-[10px] font-bold uppercase tracking-widest text-slate-500 leading-relaxed">
                    Pastikan nominal dan nomor rekening sudah benar sebelum melakukan posting transaksi.
                </p>
            </div>
        </div>
        @endif
