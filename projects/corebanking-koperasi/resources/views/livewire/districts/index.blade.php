<div class="p-0">
    <x-header title="Data Kecamatan" subtitle="Kelola daftar kecamatan" :user="$user" :role="$role">
        <x-slot:actions>
            <div class="relative">
            ...
            <table class="w-full text-left">
                <thead>
                    <tr class="bg-slate-50 border-b border-slate-200 text-slate-400 text-[10px] uppercase tracking-[0.2em] font-extrabold">
                        <th class="px-10 py-6">ID Kec</th>
                        <th class="px-6 py-6">Nama Kecamatan</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50/50 transition-colors group">
                        <td class="px-10 py-6 font-mono text-xs font-bold text-slate-400 italic">
                            {{ $item->id }}
                        </td>
                        <td class="px-6 py-6">
                        ...
                        <td class="px-6 py-6">
                            <div class="flex flex-col">
                                <span class="text-xs font-bold text-slate-900 mb-1">{{ $item->city->nama ?? 'N/A' }}</span>
                                <span class="text-[10px] text-slate-400 font-bold uppercase tracking-wider">{{ $item->province->nama ?? $item->city->province->nama ?? 'N/A' }}</span>
                            </div>
                        </td>
        ...
        <div @click.away="open = false" class="bg-white w-full max-w-lg rounded-[2.5rem] shadow-2xl overflow-hidden animate-slide-up">
            <div class="pearl-gradient p-8 text-white flex justify-between items-center relative">
                <div class="relative z-10">
                    <h3 class="text-2xl font-headline font-bold tracking-tight">{{ $editingId ? 'Edit Kecamatan' : 'Tambah Kecamatan' }}</h3>
            ...
            <form wire:submit.prevent="save" class="p-8 space-y-6">
                <div class="space-y-2">
                    <label class="text-[10px] uppercase tracking-widest font-extrabold text-slate-400 ml-1">ID Kecamatan</label>
                    <input type="text" value="{{ $editingId ?: 'Auto' }}" disabled
                        class="w-full px-5 py-3.5 bg-slate-100 border border-slate-200 rounded-2xl font-bold text-sm text-slate-500">
                </div>

                <div class="space-y-2">
                    <label class="text-[10px] uppercase tracking-widest font-extrabold text-slate-400 ml-1">Daftar Kota/Kabupaten</label>
                    <select wire:model="regency_id" class="w-full px-5 py-3.5 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-4 focus:ring-slate-900/5 focus:border-slate-900 transition-all font-bold text-sm text-slate-900">
                        <option value="">-- Pilih Kota/Kab --</option>
                        @foreach($cities as $city)
                            <option value="{{ $city->id }}">{{ $city->nama }} ({{ $city->province->nama ?? 'N/A' }})</option>
                        @endforeach
                    </select>
                    @error('regency_id') <span class="text-[10px] text-rose-500 font-bold ml-1 block">{{ $message }}</span> @enderror
                </div>

                <div class="space-y-2">
                    <label class="text-[10px] uppercase tracking-widest font-extrabold text-slate-400 ml-1">Nama Kecamatan</label>
                    <input type="text" wire:model="nama" class="w-full px-5 py-3.5 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-4 focus:ring-slate-900/5 focus:border-slate-900 transition-all font-bold text-sm text-slate-900" placeholder="Masukkan nama kecamatan">
