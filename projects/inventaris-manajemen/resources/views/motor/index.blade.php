...
@section('page-title', 'Data Inventaris Motor')
...
<div class="card-premium">
    <div class="flex flex-col md:flex-row justify-between items-center gap-4 mb-6">
        <h3 class="text-xl font-bold text-gray-900 tracking-tight"><i class="fa-solid fa-motorcycle mr-2 text-primary-600"></i> Daftar Aset Motor</h3>
    ...
    <div class="overflow-x-auto">
        <table id="table-motor" class="w-full text-left border-collapse whitespace-nowrap text-[13px]">
            <thead>
                <tr class="bg-gray-50 border-y border-gray-200">
                    <th class="px-3 py-3 text-[10px] font-bold text-gray-500 uppercase tracking-wider">Rekening / Aset</th>
                    <th class="px-3 py-3 text-[10px] font-bold text-gray-500 uppercase tracking-wider">No. Polisi / BPKB</th>
                    <th class="px-3 py-3 text-[10px] font-bold text-gray-500 uppercase tracking-wider">No. Mesin / Rangka</th>
                    <th class="px-3 py-3 text-[10px] font-bold text-gray-500 uppercase tracking-wider">Tahun / Warna</th>
                    <th class="px-3 py-3 text-[10px] font-bold text-gray-500 uppercase tracking-wider">Atas Nama</th>
...
<div id="importModal" class="hidden fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div class="relative z-10 inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
            <form action="{{ route('motor.import') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <div class="sm:flex sm:items-start">
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                            <h3 class="text-lg leading-6 font-medium text-gray-900" id="modal-title">Import Data Motor</h3>
                            <div class="mt-2">
                                <p class="text-sm text-gray-500 mb-4">Upload file Excel (.xlsx atau .csv) untuk mengimpor data kendaraan.</p>
                                <div class="mb-4">
...
<script>
    ...
    $(document).ready(function() {
        table = $('#table-motor').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                url: "{{ route('motor.data') }}",
                data: function(d) {
                    d.kantor_id = $('#filter_kantor').val();
                }
            },
            columns: [
                { data: 'rekening', name: 'rekening', className: 'px-3 py-2', render: function(data, type, row) {
                    return `<div class="font-mono text-[11px] text-gray-500">${data}</div>
                            <div class="text-[13px] text-gray-800 font-medium mt-0.5 truncate max-w-[200px]" title="${row.nama_aset}">${row.nama_aset}</div>`;
                }},
                { data: 'no_polisi', name: 'motor.no_polisi', className: 'px-3 py-2', render: function(data, type, row) {
                    let html = `<div class="font-medium text-gray-800">${data || '-'}</div>`;
                    if (row.no_bpkb && row.no_bpkb !== '-') html += `<div class="text-[11px] text-gray-400">BPKB: ${row.no_bpkb}</div>`;
                    return html;
                }},
                { data: 'no_mesin', name: 'motor.no_mesin', orderable: false, className: 'px-3 py-2', render: function(data, type, row) {
                    let html = `<div class="text-gray-700">${data || '-'}</div>`;
                    if (row.no_rangka && row.no_rangka !== '-') html += `<div class="text-[11px] text-gray-400">Rangka: ${row.no_rangka}</div>`;
                    return html;
                }},
                { data: 'tahun_pembuatan', orderable: false, className: 'px-3 py-2', render: function(data, type, row) {
                    let html = `<div class="text-gray-700">${data || '-'}</div>`;
                    if (row.warna_motor && row.warna_motor !== '-') html += `<div class="text-[11px] text-gray-400">${row.warna_motor}</div>`;
                    return html;
                }},
                { data: 'atas_nama', orderable: false, className: 'px-3 py-2 text-gray-600' },
                { 
                    data: 'id', 
                    name: 'aksi', 
                    orderable: false, 
                    ...
                    render: function(data) {
                        return `
                            <div class="flex justify-center gap-1">
                                ...
                                <a href="/motor/${data}/edit" class="w-7 h-7 rounded-md bg-blue-50 text-blue-500 hover:bg-blue-100 flex items-center justify-center transition text-xs" title="Edit">
