...
@section('page-title', 'Master Sumber Dana')

@section('content')
<div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
    <div class="flex justify-between items-center mb-6">
        <h3 class="text-lg font-semibold text-gray-800">Daftar Sumber Dana</h3>
    ...
    <div class="overflow-x-auto">
        <table id="table-sumber-dana" class="w-full text-left border-collapse">
            <thead>
                <tr class="bg-gray-50 border-y border-gray-200">
                    <th class="px-4 py-3 text-xs font-semibold text-gray-500 uppercase">Kode</th>
                    <th class="px-4 py-3 text-xs font-semibold text-gray-500 uppercase">Nama Sumber Dana</th>
...
@include('master.sumber-dana._form')
...
<script>
    let table;
    const saveUrl = "{{ route('master.sumber-dana.store') }}";
    
    $(document).ready(function() {
        table = $('#table-sumber-dana').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('master.sumber-dana.data') }}",
            columns: [
                { data: 'kode', name: 'kode' },
                { data: 'nama', name: 'nama' },
                { 
        ...
        $('#form-sumber-dana').on('submit', function(e) {
            e.preventDefault();
            
            const id = $('#sumber_dana_id').val();
            const url = id ? `/master/sumber-dana/${id}` : saveUrl;
        ...
        $('#form-sumber-dana input').on('input', function() {
    ...
    function openModal() {
        ...
        $('#sumber_dana_id').val('');
        $('#modal-title').text('Tambah Sumber Dana');
        $('.error-message').addClass('hidden');
        $('#modal-sumber-dana').removeClass('hidden');
        setTimeout(() => {
            $('#modal-sumber-dana-panel').removeClass('scale-95 opacity-0').addClass('scale-100 opacity-100');
        }, 10);
    }

    function closeModal() {
        $('#modal-sumber-dana-panel').removeClass('scale-100 opacity-100').addClass('scale-95 opacity-0');
        setTimeout(() => {
            $('#modal-sumber-dana').addClass('hidden');
        }, 200);
    }

    function editData(id) {
        showLoading();
        $.get(`/master/sumber-dana/${id}`, function(res) {
            hideLoading();
            if (res.success) {
                const data = res.data;
                $('#sumber_dana_id').val(data.id);
                $('#kode').val(data.kode);
                $('#nama').val(data.nama);
                
                $('#modal-title').text('Edit Sumber Dana');
                $('.error-message').addClass('hidden');
    ...
    function deleteData(id) {
        confirmDelete(`/master/sumber-dana/${id}`, 'table-sumber-dana');
    }
</script>
@endpush
