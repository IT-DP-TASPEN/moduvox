...
@section('title', 'Master Kantor Cabang')
@section('page-title', 'Master Kantor Cabang')

@section('content')
<div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
    <div class="flex justify-between items-center mb-6">
        <h3 class="text-lg font-semibold text-gray-800">Daftar Kantor Cabang</h3>
    ...
    <div class="overflow-x-auto">
        <table id="table-kantor" class="w-full text-left border-collapse">
            <thead>
                <tr class="bg-gray-50 border-y border-gray-200">
                    <th class="px-4 py-3 text-xs font-semibold text-gray-500 uppercase">Kode</th>
                    <th class="px-4 py-3 text-xs font-semibold text-gray-500 uppercase">Nama Kantor</th>
...
<script>
    let table;
    const saveUrl = "{{ route('master.kantor.store') }}";
    
    $(document).ready(function() {
        table = $('#table-kantor').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('master.kantor.data') }}",
            columns: [
                { data: 'kode', name: 'kode' },
                { data: 'nama', name: 'nama' },
                { 
        ...
        // Form Submit
        $('#form-kantor').on('submit', function(e) {
            e.preventDefault();
            
            const id = $('#kantor_id').val();
            const url = id ? `/master/kantor/${id}` : saveUrl;
        ...
        $('#form-kantor input').on('input', function() {
    ...
    function openModal() {
        $('#form-kantor')[0].reset();
        $('#kantor_id').val('');
        $('#modal-title').text('Tambah Kantor Cabang');
        $('.error-message').addClass('hidden');
        $('#modal-kantor').removeClass('hidden');
        setTimeout(() => {
            $('#modal-kantor-panel').removeClass('scale-95 opacity-0').addClass('scale-100 opacity-100');
        }, 10);
    }

    function closeModal() {
        $('#modal-kantor-panel').removeClass('scale-100 opacity-100').addClass('scale-95 opacity-0');
        setTimeout(() => {
            $('#modal-kantor').addClass('hidden');
        }, 200);
    }

    function editData(id) {
        showLoading();
        $.get(`/master/kantor/${id}`, function(res) {
            hideLoading();
            if (res.success) {
                const data = res.data;
                ...
                $('#modal-title').text('Edit Kantor Cabang');
                $('.error-message').addClass('hidden');
    ...
    function deleteData(id) {
        confirmDelete(`/master/kantor/${id}`, 'table-kantor');
    }
</script>
@endpush
