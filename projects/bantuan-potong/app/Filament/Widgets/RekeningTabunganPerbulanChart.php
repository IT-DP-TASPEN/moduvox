...
class RekeningTabunganPerbulanChart extends ChartWidget
{
    protected static ?string $heading = 'Grafik Saving Account Status';
    ...
    protected function getData(): array
    {
        ...
        foreach ($statuses as $status => $meta) {
            $data = $months->map(function ($month) use ($status, $user, $isSuperOrStaff, $isPusat, $isCabang) {
                $query = SavingAccount::where('status', $status);
