...
class FlaggingMutasiPerbulanChart extends ChartWidget
{
    protected static ?string $heading = 'Grafik Flagging Mutasi Status';
    ...
    protected function getData(): array
    {
        ...
        foreach ($statuses as $status => $meta) {
            $data = $months->map(function ($month) use ($status, $user, $isSuperOrStaff, $isPusat, $isCabang) {
                $query = PermintaanFlaggingMutasiTif::where('status', $status);
