...
class OpenFlaggingPerbulanChart extends ChartWidget
{
    protected static ?string $heading = 'Grafik Open Flagging Status';
    ...
    protected function getData(): array
    {
        ...
        foreach ($statuses as $status => $meta) {
            $data = $months->map(function ($month) use ($status, $user, $isSuperOrStaff, $isPusat, $isCabang) {
                $query = PermintaanOpenFlaggingTif::where('status', $status);
