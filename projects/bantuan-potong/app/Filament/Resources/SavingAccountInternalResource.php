...
use App\Filament\Resources\SavingAccountInternalResource\Pages;
use Joaopaulolndev\FilamentPdfViewer\Forms\Components\PdfViewerField;
use App\Filament\Resources\SavingAccountInternalResource\RelationManagers;
use App\Models\MarketingMaster;

class SavingAccountInternalResource extends Resource
{
    protected static ?string $model = SavingAccountInternal::class;
        ...
        return $form
            ->schema([
                Forms\Components\Section::make('Detail Saving Account')
                    ->schema([
                        Forms\Components\Select::make('saving_account_product')
                            ->label('Produk Tabungan')
                            ->relationship('savingProduct', 'product_name')
                            ->required()
                            ->inlineLabel()
                        ...
                        Forms\Components\Select::make('saving_perpouse')
                            ->label('Tujuan Pembukaan Rekening')
                            ->options(
                                AccountPerpouseMaster::pluck('perpouse_name', 'perpouse_name')
                            )
                            ->required()
                            ->inlineLabel()
                            ->reactive()
                            ...
                            ->afterStateUpdated(
                                fn(callable $set) =>
                                $set('saving_perpouse_detail', null)
                            ),

                        Forms\Components\Radio::make('saving_perpouse_detail')
                            ->label('Detail Tujuan Pembukaan Rekening')
                            ->options(function (callable $get) {
                                $perpouseName = $get('saving_perpouse');

                                if (!$perpouseName) {
                                    return [];
                                }

                                $master = AccountPerpouseMaster::where(
                                    'perpouse_name',
                                    $perpouseName
                                )->first();
                                ...
                                return AccountPerpouseDetail::where(
                                    'account_perpouse_master_id',
                                    $master->id
                                )
                                    ->pluck('detail_name', 'detail_name');
                            })
                            ->required()
                            ->inlineLabel()
                            ->visible(
                                fn(callable $get) =>
                                filled($get('saving_perpouse'))
                            )
                            ->disabled(fn() => $readonlyForApproval),
                        Forms\Components\TextInput::make('wilayah')
                            ...
                            ->columnSpanFull(),
                        Forms\Components\Select::make('nip_notas')
                            ->label('Jenis NIP/NOTAS')
                            ->required()
                            ->options([
                                'nip' => 'NIP',
                                'notas' => 'NOTAS',
                        ...
                        Forms\Components\TextInput::make('notas')
                            ->label('NIP/NOTAS Nasabah')
                            ->rules(['regex:/^[A-Za-z0-9]+$/'])
                            ->length(fn(callable $get) => match ($get('nip_notas')) {
                                'nip' => 18,
                                'notas' => 11,
                                default => null,
                            })
                            ->validationMessages([
                                ...
                                'size' => fn(callable $get) => $get('nip_notas') === 'nip'
                                    ? 'NIP harus 18 karakter'
                                    : 'Notas harus 11 karakter',
                            ])
                ...
                Forms\Components\Section::make('Rekening Transfer')
                    ->schema([
                        ...
                        Forms\Components\TextInput::make('bank_transfer')
                            ...
                            ->required()
                            ->columnSpanFull(),
                        Forms\Components\Textarea::make('tujuan_transfer')
                            ->inlineLabel()
                            ->disabled(fn() => $readonlyForApproval)
                            ->dehydrated(true)
                            ->required()
                    ...
                    ->columns(1),
                Forms\Components\Section::make('Dokumen Standing Instruction')
                    ->schema([
                        AdvancedFileUpload::make('doc_si')
                            ->label('')
                            ->required()
                            ->rules([
                                'mimes:pdf',
                    ...
                    ->schema([
                        Forms\Components\Select::make('status')
                            ...
                            ->options(function (callable $get) {
                                ...
                                } elseif ($user->hasRole(['open_table', 'maker_mitra_pusat', 'admin_support'])) {
                        ...
                        Forms\Components\Select::make('marketing_id')
                            ->label('Marketing')
                            ->options(function (?SavingAccountInternal $record) {
                                $user = auth()->user();
                        ...
                        Forms\Components\Select::make('mitra_master_id')
                            ->label('Mitra')
                            ->helperText('*Untuk pembuatan rekening Sinergi silahkan pilih selain PT Moduvox Tech ID sesuai dengan Mitra Sinergi')
                            ->relationship(
                                name: 'mitraMaster',
        ...
        return $table
            ...
            ->columns([
                Tables\Columns\TextColumn::make('wilayah')
                    ->label('Wilayah'),
                Tables\Columns\TextColumn::make('nip_notas')
                    ->label('Jenis NIP / Notas')
                ...
                Tables\Columns\TextColumn::make('tujuan_transfer')
                    ->label('Tujuan Transfer')
                    ->limit(20)
                    ->tooltip(fn($record) => $record->tujuan_transfer),

                Tables\Columns\IconColumn::make('doc_si')
                    ->label('Dokumen Standing Instructions')
                    ->icon('heroicon-o-document-text')
                    ->url(fn($record) => Storage::url($record->doc_si))
                    ->openUrlInNewTab()
                    ->tooltip('Dokumen Standing Instructions')
                    ->alignCenter(),
                ...
                Tables\Columns\TextColumn::make('mitraMaster.nama_mitra')->label('Mitra')
                    ->placeholder('Tidak diatur'),
                ...
                Tables\Filters\SelectFilter::make('mitra_master_id')
                    ...
                    ->options(
                        \App\Models\MitraMaster::query()
                            ->whereIn(
                                'id',
                                SavingAccountInternal::whereNotNull('mitra_master_id')
            ...
            ->actions([
                ...
                Tables\Actions\EditAction::make()
                    ->visible(function ($record) {
                        ...
                        if ($user->hasRole(['open_table', 'abm']) && $record->status === 'request') {
                            return true;
                        }
                        return false;
                    }),
                Tables\Actions\Action::make('continueRegisteredNotas')
                    ...
                    ->action(function ($record) {
                        ...
                        $newNotas = \Illuminate\Support\Facades\DB::transaction(function () use ($record, $oldNotas) {
                            ...
                            $record->update([
                                'status' => 'approved',
                                'keterangan_2' => "NOTAS ownership {$oldNotas} diubah ke {$newNotas}. Proses dikembalikan ke Approved.",
                            ]);
                ...
                Tables\Actions\Action::make('getCifInternal')
                    ->label('Get CIF')
                ...
                Tables\Actions\Action::make('print')
                    ...
                    ->url(fn($record) => route('form_tab_internal', $record))
                    ->openUrlInNewTab()
            ...
            ->bulkActions([
                Tables\Actions\BulkActionGroup::make([
                    Tables\Actions\DeleteBulkAction::make()->visible(function () {
                        ...
                        if ($user->hasRole(['open_table', 'abm'])) {
                            return true;
                        }
    ...
    {
        return [
            'index' => Pages\ListSavingAccountInternals::route('/'),
            'create' => Pages\CreateSavingAccountInternal::route('/create'),
            'view' => Pages\ViewSavingAccountInternal::route('/{record}'),
            'edit' => Pages\EditSavingAccountInternal::route('/{record}/edit'),
        ];
    }
}
