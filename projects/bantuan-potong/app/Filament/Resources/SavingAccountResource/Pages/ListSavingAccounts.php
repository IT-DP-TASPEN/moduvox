...
use App\Exports\SavingAccountExport;
use App\Exports\SavingAccountFincloudExport;
...
class ListSavingAccounts extends ListRecords
{
    ...
    protected function getHeaderActions(): array
    {
        return [
            ...
            Actions\Action::make('exportSaving')
                ...
                ->action(function () {
                    ...
                    return Excel::download(new SavingAccountExport($records), $filename);
                }),

            Actions\Action::make('exportSavingFc')
                ->visible(fn() => Auth::user()->hasRole(['staff_bosche', 'super_admin']))
                ->label('Export Saving Data Fincloud')
                ->icon('heroicon-o-arrow-down-tray')
                ->color('success')
                ->requiresConfirmation()
                ->modalHeading('Export')
                ...
                ->action(function () {
                    ...
                    $filename = 'savingfincloud' . now()->format('Ymd_His') . '.xlsx';

                    return Excel::download(new SavingAccountFincloudExport($records), $filename);
                }),
        ];
    }
